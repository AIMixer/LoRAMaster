...
HUNYUAN_SETTINGS_FILE = 'hunyuan_settings.toml'
# 预缓存进程
cache_process = None
...
def load_settings() -> dict:
    if os.path.exists(HUNYUAN_SETTINGS_FILE):
        try:
            with open(HUNYUAN_SETTINGS_FILE, "r", encoding="utf-8") as f:
...
def save_settings():
    try:
        with open(HUNYUAN_SETTINGS_FILE, "w", encoding="utf-8") as f:
            toml.dump(hunyuan_training_settings, f)
    except Exception as e:
        print(f"[WARN] 保存 settings.toml 失败: {e}")
def bind_setting(ui_element, key):
    """将 UI 控件的值绑定到 hunyuan_training_settings[key] 并自动保存"""
...
def update_setting(key, e):
    ...
    hunyuan_training_settings[key] = value
    save_settings()
    preview_settings()
...
def preview_settings():
    ...
    toml_str = toml.dumps(hunyuan_training_settings)
...
hunyuan_training_settings = load_settings()
preview_settings()
...
def start_pre_caching():
    ...
    dataset_config = hunyuan_training_settings['dataset_config']
    vae_path = hunyuan_training_settings['vae_path']
    clip_model_path = hunyuan_training_settings['clip_model_path']
    text_encoder_model_path = hunyuan_training_settings['text_encoder_model_path']
    fp8 = hunyuan_training_settings['fp8']
    batch_size = hunyuan_training_settings['batch_size']
    ...
    cache_latents_path = os.path.join(MUSUBI_DIR, "cache_latents.py")
    cache_text_encoder_path = os.path.join(MUSUBI_DIR, "cache_text_encoder_outputs.py")
    ...
    cache_text_encoder_cmd = [
        ...
        "--dataset_config", dataset_config,
        "--text_encoder1", text_encoder_model_path,
        "--text_encoder2", clip_model_path,
        "--batch_size", batch_size
    ]

    if fp8:
        cache_text_encoder_cmd.extend(['--fp8_llm'])
...
def make_prompt_file(
    ...
    if prompt_file_upload and os.path.isfile(prompt_file_upload):
        ...
        default_prompt_path = "./hunyuan_prompt_file.txt"
...
def run_wan_training():
    dataset_config = hunyuan_training_settings['dataset_config']
    clip_model_path = hunyuan_training_settings['clip_model_path']
    dit_weights_path = hunyuan_training_settings['dit_weights_path']
    learning_rate = hunyuan_training_settings['learning_rate']
    gradient_accumulation_steps = hunyuan_training_settings['gradient_accumulation_steps']
    network_dim = hunyuan_training_settings['network_dim']
    timestep_sampling = hunyuan_training_settings['timestep_sampling']
    discrete_flow_shift = hunyuan_training_settings['discrete_flow_shift']
    max_train_epochs = hunyuan_training_settings['max_train_epochs']
    save_every_n_epochs = hunyuan_training_settings['save_every_n_epochs']
    save_every_n_steps = hunyuan_training_settings['save_every_n_steps']
    output_dir = hunyuan_training_settings['output_dir']
    output_name = hunyuan_training_settings['output_name']
    enable_low_vram = hunyuan_training_settings['enable_low_vram']
    blocks_to_swap = hunyuan_training_settings['blocks_to_swap']
    use_network_weights = hunyuan_training_settings['use_network_weights']
    network_weights_path = hunyuan_training_settings['network_weights_path']
    generate_samples = hunyuan_training_settings['generate_samples']
    sample_prompt_text = hunyuan_training_settings['sample_prompt_text']
    # sample_image_path = hunyuan_training_settings['sample_image_path']
    sample_w = hunyuan_training_settings['sample_w']
    sample_h = hunyuan_training_settings['sample_h']
    sample_frames = hunyuan_training_settings['sample_frames']
    sample_seed = hunyuan_training_settings['sample_seed']
    sample_steps = hunyuan_training_settings['sample_steps']
    custom_prompt_txt = hunyuan_training_settings['custom_prompt_txt']
    custom_prompt_path = hunyuan_training_settings['custom_prompt_path']
    sample_every_n_epochs = hunyuan_training_settings['sample_every_n_epochs']
    sample_every_n_steps = hunyuan_training_settings['sample_every_n_steps']
    sample_vae_path = hunyuan_training_settings['vae_path']
    fp8 = hunyuan_training_settings['fp8']
    num_cpu_threads_per_process = hunyuan_training_settings['num_cpu_threads_per_process']
    num_processes = hunyuan_training_settings['num_processes']
    attention_implementation = hunyuan_training_settings['attention_implementation']
    optimizer_type = hunyuan_training_settings['optimizer_type']
    max_data_loader_n_workers = hunyuan_training_settings['max_data_loader_n_workers']
    log_type = hunyuan_training_settings['log_type']
    log_prefix = hunyuan_training_settings['log_prefix']
    log_dir = hunyuan_training_settings['log_dir']
    log_tracker_name = hunyuan_training_settings['log_tracker_name']
    offload_inactive_dit = hunyuan_training_settings['offload_inactive_dit']
    mixed_precision = hunyuan_training_settings['mixed_precision']
    sample_at_first = hunyuan_training_settings['sample_at_first']
    text_encoder_model_path = hunyuan_training_settings['text_encoder_model_path']
    ...
    # 拼接 musubi-tuner 目录下的脚本路径
    MUSUBI_DIR = os.path.join(base_dir, 'musubi-tuner','src','musubi_tuner')
    wan_train_network_path = os.path.join(MUSUBI_DIR, "hv_train_network.py")
    ...
    def run_and_stream_output():
        ...
        if return_code != 0:
            ...
            try:
                ...
                print(f"{e}")
            # 自动关机
            if hunyuan_training_settings['auto_shutdown']:
...
def draw_ui():
    ui.label('Hunyuan Video LoRA训练').classes('text-2xl font-bold')
        ...
        with ui.column().classes('w-3/4'):
            with ui.list().props('bordered separator').classes('w-full'):
                ...
                with ui.item():
                    with ui.item_section():
                        ...
                        ui.item_label('混元Video T2V底膜，如mp_rank_00_model_states.pt').props('caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        dit_weights_path = ui.input(
                            placeholder='如I:\\train_models\\hunyuan\\如mp_rank_00_model_states.pt',
                            value=hunyuan_training_settings['dit_weights_path']).props(
                            'rounded outlined dense').classes('w-full')
                        bind_setting(dit_weights_path, 'dit_weights_path')
                with ui.item():
                    with ui.item_section():
                        ui.item_label('VAE File Path / VAE文件路径')
                        ui.item_label('Hunyuan VAE文件的绝对路径，如: I:\\train_models\\hunyuan\\vae.pt').props('caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        vae_path = ui.input(placeholder='如: I:\\train_models\\hunyuan\\vae.pt',
                                            value=hunyuan_training_settings["vae_path"]).classes('w-full').props('rounded outlined dense')
                        bind_setting(vae_path, 'vae_path')
                with ui.item():
                    with ui.item_section():
                        ui.item_label('CLIP_L Model Path / CLIP_L模型路径')
                        ui.item_label('clip_l.safetensors').props('caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        clip_model_path = ui.input(
                            placeholder='如: I:\\train_models\\hunyuan\\clip_l.safetensors',
                            value=hunyuan_training_settings["clip_model_path"]).props(
                            'rounded outlined dense').classes('w-full')
                        bind_setting(clip_model_path, 'clip_model_path')
                with ui.item():
                    with ui.item_section():
                        ui.item_label('Text Encoder Model Path / Text Encoder 模型路径')
                        ui.item_label('T5模型文件的绝对路径，如: I:\\train_models\\hunyuan\\llava_llama3_fp16.safetensors').props('caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        t5_path = ui.input(placeholder='如: I:\\train_models\\hunyuan\\llava_llama3_fp16.safetensors',
                                           value=hunyuan_training_settings["text_encoder_model_path"]).classes('w-full').props('rounded outlined dense')
                        bind_setting(t5_path, 'text_encoder_model_path')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side'):
                        ...
                        fp8 = ui.switch(value=hunyuan_training_settings['fp8'])
                ...
                with ui.item():
                    with ui.item_section():
                        ...
                        ui.item_label('toml配置文件的绝对路径，如:E:\\hunyuan_lora_train\\config.toml').props('caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        dataset_config = ui.input(placeholder='如:E:\\hunyuan_lora_train\\config.toml',
                                                  value=hunyuan_training_settings["dataset_config"]).classes(
            ...
            with ui.list().props('bordered separator').classes('w-full'):
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side'):
                        batch_size = ui.number(placeholder='同时送入 batch_size 条文本样本到 Text Encoder 编码器',
                                               value=hunyuan_training_settings["batch_size"]).style('width:200px').props('rounded outlined dense')
            ...
            with ui.list().props('bordered separator').classes('w-full'):
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        max_train_epochs = ui.number(value=hunyuan_training_settings['max_train_epochs']).props('rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        learning_rate = ui.input(value=hunyuan_training_settings['learning_rate']).props('rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        optimizer_type = ui.select(
                            ...
                            value=hunyuan_training_settings['optimizer_type']).props(
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        network_dim = ui.number(value=hunyuan_training_settings['network_dim']).props('rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        mixed_precision = ui.select(
                            ['fp16', 'bf16'],
                            value=hunyuan_training_settings['mixed_precision']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        gradient_steps = ui.number(value=hunyuan_training_settings['gradient_accumulation_steps']).props('rounded outlined dense').classes('w-1/2')
                        bind_setting(gradient_steps, 'gradient_accumulation_steps')
                with ui.item():
                    with ui.item_section():
                        ui.item_label('Timestep Sampling / 时间步采样')
                    with ui.item_section().props('side').classes('w-1/2'):
                        timestep_sampling = ui.input(value=hunyuan_training_settings['timestep_sampling']).props('rounded outlined dense').classes('w-1/2')
                        bind_setting(timestep_sampling, 'timestep_sampling')

                with ui.item():
                    with ui.item_section():
                        ui.item_label('Discrete Flow Shift / 离散流移位')
                        ui.item_label('建议配置：7。不一定完全一致，可以用作参考').props('caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        discrete_shift = ui.number(value=hunyuan_training_settings['discrete_flow_shift']).props('rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        max_data_loader_n_workers = ui.number(value=hunyuan_training_settings['max_data_loader_n_workers']).props('rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        attention_implementation = ui.select(
                            ['sdpa', 'xformers'],
                            value=hunyuan_training_settings['attention_implementation']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        enable_low_vram = ui.switch(value=hunyuan_training_settings['enable_low_vram']).props('outlined')
                ...
                with ui.item().bind_visibility_from(enable_low_vram, 'value'):
                    with ui.item_section():
                        ...
                        ui.item_label(
                            ...
                            'caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        blocks_to_swap = ui.number(value=hunyuan_training_settings['blocks_to_swap']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        num_cpu_threads_per_process = ui.number(
                            value=hunyuan_training_settings['num_cpu_threads_per_process']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        num_processes = ui.number(value=hunyuan_training_settings['num_processes']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        use_network_weights = ui.switch(value=hunyuan_training_settings['use_network_weights']).props(
                            'outlined')
                ...
                with ui.item().bind_visibility_from(use_network_weights, 'value'):
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        network_weights_path = ui.input(placeholder='Input weights file path / 请输入权重文件路径',
                                                        value=hunyuan_training_settings['network_weights_path']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        generate_samples = ui.switch(value=hunyuan_training_settings['generate_samples'])
                ...
                with ui.item().bind_visibility_from(generate_samples, 'value'):
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        sample_at_first = ui.switch(value=hunyuan_training_settings['sample_at_first'])
                        bind_setting(sample_at_first, 'sample_at_first')

                with ui.item().bind_visibility_from(generate_samples, 'value'):
                    with ui.row().classes('w-full no-wrap gap-4'):
                        sample_epoch = ui.number('Sample Every N Epochs / 每N个轮次采样一次',
                                                 value=hunyuan_training_settings['sample_every_n_epochs']).props(
                            'outlined').classes('w-1/2')
                        sample_step = ui.number('Sample Every N Steps / 每N步采样一次',
                                                value=hunyuan_training_settings['sample_every_n_steps']).props(
                            'outlined').classes('w-1/2')
                ...
                with ui.item().bind_visibility_from(generate_samples, 'value'):
                    with ui.row().classes('w-full no-wrap gap-4'):
                        sample_prompt = ui.input('Prompt Text / 提示词',
                                                 value=hunyuan_training_settings['sample_prompt_text']).props(
                            'outlined').classes('w-full')
                        bind_setting(sample_prompt, 'sample_prompt_text')
                # with ui.item().bind_visibility_from(generate_samples, 'value'):
                #     with ui.row().classes('w-full no-wrap gap-4'):
                #         sample_image_path = ui.input('Image Path / 图片路径',
                #                                      value=hunyuan_training_settings['sample_image_path']).props(
                #             'outlined').classes('w-full')
                #         bind_setting(sample_image_path, 'sample_image_path')
                with ui.item().bind_visibility_from(generate_samples, 'value'):
                    with ui.row().classes('w-full no-wrap gap-4'):
                        sample_w = ui.number('Width (w) / 宽度', value=hunyuan_training_settings['sample_w']).props(
                            'outlined').classes(
                            'w-1/3')
                        sample_h = ui.number('Height (h) / 高度', value=hunyuan_training_settings['sample_h']).props(
                            'outlined').classes(
                            'w-1/3')
                        sample_frames = ui.number('Frames (f) / 帧数',
                                                  value=hunyuan_training_settings['sample_frames']).props(
                ...
                with ui.item().bind_visibility_from(generate_samples, 'value'):
                    with ui.row().classes('w-full no-wrap gap-4'):
                        sample_seed = ui.number('Seed (d) / 种子', value=hunyuan_training_settings['sample_seed']).props(
                            'outlined').classes('w-1/2')
                        sample_steps = ui.number('Steps (s) / 步数', value=hunyuan_training_settings['sample_steps']).props(
                            'outlined').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        output_dir = ui.input(value=hunyuan_training_settings['output_dir']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        output_name = ui.input(value=hunyuan_training_settings['output_name']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        save_epochs = ui.number(value=hunyuan_training_settings['save_every_n_epochs']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        save_steps = ui.number(value=hunyuan_training_settings['save_every_n_steps']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        log_type = ui.select(
                            ['tensorboard', 'wandb'],
                            value=hunyuan_training_settings['log_type']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        aush = ui.switch(value=hunyuan_training_settings['auto_shutdown']).props('outlined')
