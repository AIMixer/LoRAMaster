...
WAN_SETTINGS_FILE = 'kontext_settings.toml'
# 预缓存进程
cache_process = None
...
def save_settings():
    try:
        with open(WAN_SETTINGS_FILE, "w", encoding="utf-8") as f:
            toml.dump(kontext_training_settings, f)
    except Exception as e:
        print(f"[WARN] 保存 settings.toml 失败: {e}")
def bind_setting(ui_element, key):
    """将 UI 控件的值绑定到 kontext_training_settings[key] 并自动保存"""
...
def update_setting(key, e):
    ...
    kontext_training_settings[key] = value
    save_settings()
    preview_settings()
...
def preview_settings():
    ...
    toml_str = toml.dumps(kontext_training_settings)
...
def start_pre_caching():
    ...
    batch_size = kontext_training_settings['batch_size']
    ...
    wan_cache_latents_path = os.path.join(MUSUBI_DIR, "flux_kontext_cache_latents.py")
    wan_cache_text_encoder_path = os.path.join(MUSUBI_DIR, "flux_kontext_cache_text_encoder_outputs.py")
...
def run_wan_training():
    ...
    output_dir = kontext_training_settings['output_dir']
    output_name = kontext_training_settings['output_name']
    enable_low_vram = kontext_training_settings['enable_low_vram']
    blocks_to_swap = kontext_training_settings['blocks_to_swap']
    use_network_weights = kontext_training_settings['use_network_weights']
    network_weights_path = kontext_training_settings['network_weights_path']
    generate_samples = kontext_training_settings['generate_samples']
    sample_prompt_text = kontext_training_settings['sample_prompt_text']
    ...
    custom_prompt_path = kontext_training_settings['custom_prompt_path']
    sample_every_n_epochs = kontext_training_settings['sample_every_n_epochs']
    ...
    log_type = kontext_training_settings['log_type']
    log_prefix = kontext_training_settings['log_prefix']
    log_dir = kontext_training_settings['log_dir']
    log_tracker_name = kontext_training_settings['log_tracker_name']
    offload_inactive_dit = kontext_training_settings['offload_inactive_dit']
    ...
    custom_params = kontext_training_settings['custom_params']
    ...
    def run_and_stream_output():
        ...
        if return_code != 0:
            ...
            try:
                ...
                print(f"{e}")
            # 自动关机
            if kontext_training_settings['auto_shutdown']:
        ...
        with ui.column().classes('w-3/4'):
            with ui.list().props('bordered separator').classes('w-full'):
                ...
                with ui.item():
                    with ui.item_section():
                        ...
                        ui.item_label('Kontext Dev模型，如flux1-kontext-dev.safetensors').props('caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        dit_weights_path = ui.input(
                            placeholder='如I:\\train_models\\kontext\\如flux1-kontext-dev.safetensors',
                ...
                with ui.item():
                    with ui.item_section():
                        ...
                        ui.item_label('Kontext ae.safetensors文件的绝对路径，如:I:\\train_models\\kontext\\ae.safetensors').props('caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        vae_path = ui.input(placeholder='如: E:\\models\\vae\\Wan2.1_VAE.pth',
                                            value=kontext_training_settings["vae_path"]).classes('w-full').props('rounded outlined dense')
                        bind_setting(vae_path, 'vae_path')
                with ui.item():
                    with ui.item_section():
                        ui.item_label('T5 Model Path / T5模型路径')
                        ui.item_label('T5模型文件的绝对路径，如: I:\\train_models\\kontext\\t5xxl_fp16.safetensors').props('caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        t5_path = ui.input(placeholder='如：I:\\train_models\\kontext\\t5xxl_fp16.safetensors',
                                           value=kontext_training_settings["t5_path"]).classes('w-full').props('rounded outlined dense')
                        bind_setting(t5_path, 't5_path')
                with ui.item():
                    with ui.item_section():
                        ui.item_label('CLIP_L Model Path / clip_l模型路径')
                        ui.item_label('Kontext的clip_l.safetensors').props('caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        clip_model_path = ui.input(
                            placeholder='如: I:\\train_models\\kontext\\clip_l.safetensors',
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side'):
                        batch_size = ui.number(placeholder='同时送入 batch_size 条文本样本到 T5 编码器',
                                               value=kontext_training_settings["batch_size"]).style('width:200px').props('rounded outlined dense')
            ...
            with ui.list().props('bordered separator').classes('w-full'):
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        max_train_epochs = ui.number(value=kontext_training_settings['max_train_epochs']).props('rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        learning_rate = ui.input(value=kontext_training_settings['learning_rate']).props('rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        network_dim = ui.number(value=kontext_training_settings['network_dim']).props('rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        mixed_precision = ui.select(
                            ['fp16', 'bf16'],
                            value=kontext_training_settings['mixed_precision']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        gradient_steps = ui.number(value=kontext_training_settings['gradient_accumulation_steps']).props('rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        max_data_loader_n_workers = ui.number(value=kontext_training_settings['max_data_loader_n_workers']).props('rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        attention_implementation = ui.select(
                            ['sdpa', 'xformers'],
                            value=kontext_training_settings['attention_implementation']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        enable_low_vram = ui.switch(value=kontext_training_settings['enable_low_vram']).props('outlined')
                ...
                with ui.item().bind_visibility_from(enable_low_vram, 'value'):
                    with ui.item_section():
                        ...
                        ui.item_label(
                            ...
                            'caption')
                    with ui.item_section().props('side').classes('w-1/2'):
                        blocks_to_swap = ui.number(value=kontext_training_settings['blocks_to_swap']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        num_cpu_threads_per_process = ui.number(
                            value=kontext_training_settings['num_cpu_threads_per_process']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        num_processes = ui.number(value=kontext_training_settings['num_processes']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        use_network_weights = ui.switch(value=kontext_training_settings['use_network_weights']).props(
                            'outlined')
                ...
                with ui.item().bind_visibility_from(use_network_weights, 'value'):
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        network_weights_path = ui.input(placeholder='Input weights file path / 请输入权重文件路径',
                                                        value=kontext_training_settings['network_weights_path']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        output_dir = ui.input(value=kontext_training_settings['output_dir']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        output_name = ui.input(value=kontext_training_settings['output_name']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        save_epochs = ui.number(value=kontext_training_settings['save_every_n_epochs']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        save_steps = ui.number(value=kontext_training_settings['save_every_n_steps']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        log_type = ui.select(
                            ['tensorboard', 'wandb'],
                            value=kontext_training_settings['log_type']).props(
                            'rounded outlined dense').classes('w-1/2')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        custom_params = ui.input(value=kontext_training_settings['custom_params']).props(
                            'rounded outlined dense').classes('w-full')
                ...
                with ui.item():
                    ...
                    with ui.item_section().props('side').classes('w-1/2'):
                        aush = ui.switch(value=kontext_training_settings['auto_shutdown']).props('outlined')
